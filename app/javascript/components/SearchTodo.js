...
function SearchTodo(props) {
	...
	function markTodo(todo_id, status) {
		// Force a refresh of data from server if a todo has been marked as completed.
	...
	return (
		<div>
			<Grid padded>
				<Grid.Column>
					<Input fluid placeholder="Search..." onChange={(e, { value }) => setSearch(value)} />
				</Grid.Column>
			</Grid>
			<Grid padded>
				<Grid.Column>
						...
						{todos.map(todo => (
							<Item>
								<Item.Content>
									...
									<Item.Extra>
										<Button floated="right" negative onClick={() => deleteTodo(todo.id)}>
											Delete
										</Button>
										<Button
											floated="right"
